...
from metric.loss import HardDarkRank, Similarity, L2Triplet, AttentionTransfer ,AOMD
...
parser.add_argument('--sp_ratio', default=1, type=float)
...
# parser.add_argument('--teacher_load', default='/media/ahu/30c5b1f3-ab0d-6f47-99ef-060ca58051ed/aomd_classfication/cars196_resnet50_512/best.pth', required=True)
parser.add_argument('--teacher_load', default='/media/ahu/30c5b1f3-ab0d-6f47-99ef-060ca58051ed/aomd_classfication/cars196_resnet50_512/best.pth')
parser.add_argument('--teacher_l2normalize', choices=['true', 'false'], default='true')
parser.add_argument('--teacher_embedding_size', default=512, type=int)
...
parser.add_argument('--save_dir', default='cars196_student_resnet18_128')
parser.add_argument('--load', default=None)

''' aomd '''
parser.add_argument('--aomd', default=True,help='useing AOMD distillation')
parser.add_argument('--aomd_ratio', default=0, type=float)
...
def train(loader, ep):
    ...
    for images, labels in train_iter:
        ...
        aomd_loss = 0.0
        if opts.aomd:
            t_feats_list = [t_b1, t_b2, t_b3, t_b4]
            s_list = [b1, b2, b3, b4]
            s_feats_list =[]
            for idx,s_feat in zip(range(4),s_list):
                new_s_faet = student.stu_layers_feats_adap[idx](s_feat)
                s_feats_list.append(new_s_faet)

            aomd_loss = opts.aomd_ratio * aomd_criterion(t_feats_list,s_feats_list)



        triplet_loss = opts.triplet_ratio * triplet_criterion(e, labels)
        sp_loss = opts.sp_ratio * sum(sp_criterion(e, t_e))/len(sp_criterion(e, t_e))
        dark_loss = opts.dark_ratio * dark_criterion(e, t_e)

        loss = triplet_loss + sp_loss + dark_loss + at_loss + aomd_loss
        ...
        aomd_loss_all.append(aomd_loss.item())
        loss_all.append(loss.item())

        train_iter.set_description("[Train][Epoch %d] Triplet: %.5f, CC: %5f ,Dark: %5f, At: %5f ,Aomd: %5f" %
                                   (ep, triplet_loss.item(), sp_loss.item(), dark_loss.item(), at_loss.item(), aomd_loss.item()))
    print('[Epoch %d] Loss: %.5f, Triplet: %.5f, CC: %.5f, Dark: %.5f , At: %.5f Aomd: %.5f\n' %\
          (ep, torch.Tensor(loss_all).mean(), torch.Tensor(triplet_loss_all).mean(),
           torch.Tensor(sp_loss_all).mean(),
           torch.Tensor(dark_loss_all).mean(),
           torch.Tensor(at_loss_all).mean(),torch.Tensor(aomd_loss_all).mean()))
