...
from metric.loss import HardDarkRank, Similarity, L2Triplet, FitNet
...
def train(loader, ep):
    ...
    for images, labels in train_iter:
        ...
        sp_loss = opts.sp_ratio * sp_criterion(e, t_e)

        dark_loss = opts.dark_ratio * dark_criterion(e, t_e)

        loss = triplet_loss + sp_loss + dark_loss
        ...
        train_iter.set_description("[Train][Epoch %d] Triplet: %.5f, SP: %.5f,  Dark: %5f" %
                                   (ep, triplet_loss.item(), sp_loss.item(),  dark_loss.item()))
    print('[Epoch %d] Loss: %.5f, Triplet: %.5f, SP: %.5f,  Dark: %.5f \n' %\
          (ep, torch.Tensor(loss_all).mean(), torch.Tensor(triplet_loss_all).mean(),
           torch.Tensor(sp_loss_all).mean(),  torch.Tensor(dark_loss_all).mean()))
